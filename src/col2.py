...
class colony(Sprite):
    ...
    def show(self):
        ...
        colimage.get_rect(center=(self.pos))
        ...
        healthbar = pygame.Surface(((((colony.SIZE[0]*2)/100)*self.health), 9),pygame.SRCALPHA)
        healthbar.fill(self.healthcheck())
        font = pygame.font.Font(None,30)
        text_suf = font.render(str(ocnum), 1, colour["ht"])
        tex_pos = (self.pos[0]-colony.SIZE[0]/4,self.pos[1]-colony.SIZE[1])
        
        #writes to screen
        self.screen.blit(text_suf,tex_pos)
        self.screen.blit(healthbg,(self.pos[0]-colony.SIZE[0],self.pos[1]+5))
        self.screen.blit(healthbar,(self.pos[0]-colony.SIZE[0],self.pos[1]+5))
